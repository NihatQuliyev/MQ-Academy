...
export default function CareersContent({ vacancies }: { vacancies: Vacancy[] }) {
  const { lang } = useLang();
      ...
      <section className="px-6 md:px-12 pb-24">
        {vacancies.length === 0 ? (
          <div className="bg-white border border-ink/[0.08] rounded-2xl p-16 text-center">
            ...
            <h3 className="text-xl font-semibold text-ink mb-2">{tr.emptyTitle}</h3>
            <p className="text-sm text-muted">{tr.emptySub}</p>
          </div>
        ) : (
        ...
        <div className="bg-orange-tint border border-orange-soft rounded-2xl p-8 flex flex-wrap items-center gap-7">
          ...
          <div className="flex-1 min-w-64">
            <h3 className="text-xl font-semibold text-ink mb-1 tracking-tight">{tr.ctaTitle}</h3>
            <p className="text-sm text-ink-3">{tr.ctaSub}</p>
          </div>
        </div>
      </section>
    </>
