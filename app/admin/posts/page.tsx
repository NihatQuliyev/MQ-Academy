...
export default function AdminPosts() {
  ...
  const uploadImage = async (file: File) => {
    if (file.size > 10 * 1024 * 1024) {
      flash("Fayl ölçüsü 10MB-dan çox ola bilməz");
      return;
    }
    setUploading(true);
    try {
      // İmza al
      const sigRes = await fetch("/api/sign-upload?folder=mq-posts");
      const { timestamp, signature, folder, cloudName, apiKey } = await sigRes.json();

      // Birbaşa Cloudinary-ə yüklə (Vercel body limitini keçmir)
      ...
      if (data.secure_url) {
        ...
        flash("Şəkil yüklənmədi: " + (data.error?.message || "Naməlum xəta"));
      }
    } catch {
      flash("Şəkil yüklənmədi. İnternet bağlantısını yoxlayın.");
    } finally {
      setUploading(false);
        ...
        <form onSubmit={save} className="p-7 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select className="field-input" value={cats.includes(form.category) ? form.category : "Digər"}
                onChange={(e) => setForm({ ...form, category: e.target.value === "Digər" ? "" : e.target.value })}>
                {cats.map((c) => <option key={c}>{c}</option>)}
              </select>
              {(!cats.includes(form.category) || form.category === "") && (
                <input className="field-input mt-2" value={form.category === "" || !cats.includes(form.category) ? form.category : ""}
                  onChange={(e) => setForm({ ...form, category: e.target.value })}
                  placeholder="Kateqoriya adını yazın..." required />
              )}
            </div>
            <div>
